...
class ManiuplatorModel:
    def __init__(self, Tp, m3=0, r3=0.01):
        self.Tp = Tp
        self.l1 = 0.5
        self.r1 = 0.01
        self.m1 = 1.
    ...
    def M(self, x):
        ...
        alpha = self.m1 * (self.l1 / 2) ** 2 + self.I_1 + self.m2 * self.l1 ** 2 + self.m2 * (self.l2 / 2) ** 2 + self.I_2 + self.m3 * self.l1 ** 2 + self.m3 * self.l2 ** 2 + self.I_3
        beta = self.m2 * self.l1 * (self.l2 / 2) + self.m3 * self.l1 * self.l2
        gamma = self.m2 * (self.l2 / 2) ** 2 + self.m3 * self.l2 ** 2 + self.I_2 + self.I_3
